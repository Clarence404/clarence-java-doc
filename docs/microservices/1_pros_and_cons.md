...
![monolithic-application-architecture-diagram](../assets/springcloud/monolithic-application-architecture-diagram.png)
...
![deal-with-complex things-1](../assets/springcloud/deal-with-complex-things-1.png)
...
![deal-with-complex-things-2](../assets/springcloud/deal-with-complex-things-2.png)
...
![deal-with-complex-things-3](../assets/springcloud/deal-with-complex-things-3.png)
...
![deal-with-complex-things-4](../assets/springcloud/deal-with-complex-things-4.png)
...
迁移单体式应用到微服务架构意味着一系列现代化过程。一个策略是：不要大规模（big bang）重写代码。重写代码听起来很不错，但实际上充满了风险最终可能会失败，就如 Martin Fowler 所说：

> "the only thing a Big Bang rewrite guarantees is a Big Bang!"

应该采取逐步迁移的策略，Martin Fowler 将这种策略称为**绞杀（Strangler）应用**，围绕着传统应用开发新型微服务，传统应用渐渐退出舞台。

### 策略 1 — 停止挖掘

停止让单体应用继续变大。开发新功能时不应该为旧单体应用添加新代码，最佳方法是将新功能开发成独立微服务。

### 策略 2 — 将前端和后端分离

将表现层与业务逻辑、数据访问层分开，形成两个可独立部署的应用，是减小单体复杂度的有效策略。

### 策略 3 — 抽出服务

从单体应用中逐步抽取模块成为独立微服务。优先抽取：
- **频繁变化的模块**：加速独立部署
- **资源消耗大的模块**：便于针对性扩容
- **边界清晰的模块**：降低迁移风险
