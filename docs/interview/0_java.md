...
`@FunctionalInterface` 是 **Java 8** 引入的注解，用来标记 **函数式接口**。

> **函数式接口**：**只包含一个抽象方法**的接口，主要用来支持 Lambda 表达式。
...
- `Greeting` 接口只允许**一个抽象方法**，多了就会报错。
- Lambda **简化了匿名内部类**的写法，更简洁易读。
...
> **Lambda 其实是“匿名内部类”的语法糖** —— 底层执行时会转成 **内部类** 形式。
